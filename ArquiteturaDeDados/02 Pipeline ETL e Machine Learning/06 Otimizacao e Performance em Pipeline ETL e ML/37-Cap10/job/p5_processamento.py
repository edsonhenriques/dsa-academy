...
def dsa_limpa_transforma_dados(spark):
	
	# Pasta raiz no HDFS
	path = "/opt/spark/data/"

	# Grava no log
	dsa_grava_log("Log DSA - Importando os dados...")

	# Carrega o arquivo CSV
	reviews = spark.read.csv(path + 'dataset.csv', header=True, escape="\"")

	# Grava no log
	dsa_grava_log("Log DSA - Dados Importados com Sucesso.")
	dsa_grava_log("Log DSA - Total de Registros: " + str(reviews.count()))
	dsa_grava_log("Log DSA - Verificando se Existem Dados Nulos.")
	...
	if (len(null_columns_calc_list) > 0):
		...
		dsa_grava_log("Dados nulos excluídos")
		dsa_grava_log("Log DSA - Total de Registros Depois da Limpeza: " + str(reviews.count()))
	else:
		dsa_grava_log("Log DSA - Valores Ausentes Nao Foram Detectados.")

	# Grava no log
	dsa_grava_log("Log DSA - Verificando o Balanceamento de Classes.")
	...
	# Grava no log
	dsa_grava_log("Log DSA - Existem " + str(count_positive_sentiment) + " reviews positivos e " + str(count_negative_sentiment) + " reviews negativos.")

	# Cria o dataframe
	df = reviews

	# Grava no log
	dsa_grava_log("Log DSA - Transformando os Dados.")
	...
	# Grava no log
	dsa_grava_log("Log DSA - Limpeza dos Dados.")
	...
	# Grava no log
	dsa_grava_log("Log DSA - Os Dados de Texto Foram Limpos.")
	dsa_grava_log("Log DSA - Tokenizando os Dados de Texto.")
	...
	# Grava no log
	dsa_grava_log("Log DSA - Removendo Stop Words.")
	...
	# Grava no log
	dsa_grava_log("Log DSA - Aplicando HashingTF.")
	...
	# Grava no log
	dsa_grava_log("Log DSA - Aplicando IDF.")
	...
	# Grava no log
	dsa_grava_log("Log DSA - Aplicando Word2Vec.")
	...
	# Grava no log
	dsa_grava_log("Log DSA - Padronizando os Dados com MinMaxScaler.")
	...
	# Grava no log
	dsa_grava_log("Log DSA - Salvando os Dados Limpos e Transformados.")
	...
	dsa_grava_log("Log DSA - Dados Salvos com Sucesso.")
