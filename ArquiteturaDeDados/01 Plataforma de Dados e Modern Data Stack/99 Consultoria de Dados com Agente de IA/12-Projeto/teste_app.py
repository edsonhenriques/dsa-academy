...
def dsa_extrai_dados(ticker, period="3mo"):
    try:
        # Cria um objeto Ticker do Yahoo Finance para a ação especificada
        stock = yf.Ticker(ticker)

        # Adiciona um pequeno atraso para evitar o limite de requisições do Yahoo Finance
        # Este delay só ocorre se o cache do Streamlit não for utilizado (primeira requisição ou após o TTL)
        time.sleep(15) 
        ...
        st.error("Erro: Limite de requisições excedido pelo Yahoo Finance. Por favor, aguarde alguns minutos e tente novamente.")
        return None # Retorna None para indicar que a extração falhou
    except Exception as e:
        st.error(f"Ocorreu um erro ao extrair dados para {ticker}: {e}")
...
def dsa_plot_stock_price(hist, ticker):
    ...
    # Não há necessidade de time.sleep() aqui, pois não há chamada externa
...
def dsa_plot_candlestick(hist, ticker):
    ...
    # Não há necessidade de time.sleep() aqui
...
Mais tickers podem ser encontrados aqui: [https://stockanalysis.com/list/nasdaq-stocks/](https://stockanalysis.com/list/nasdaq-stocks/)
...
if st.button("Analisar"):
    ...
    if ticker:
        ...
        with st.spinner("Buscando os Dados em Tempo Real. Aguarde..."):
            ...
            # Verifica se os dados foram extraídos com sucesso antes de prosseguir
            if hist is not None:
                ...
                # Mensagem de erro já foi exibida na função dsa_extrai_dados
                st.warning("Não foi possível carregar os dados históricos. Por favor, tente novamente mais tarde.")
    else:
